...
subpaths=["grobid-dictionaries_data\EEBD\dataset", "grobid-dictionaries_data\MxSp\dataset", "grobid-dictionaries_data\FangFr\dataset", "grobid-dictionaries_data\FrFang\dataset", "grobid-dictionaries_data\DLF\dataset"]
evpath = [ ]
def ask(subpaths):
    #slashpath=[]
#for i in range(len(subpaths)):
 #   slashpath.append(subpaths[i].replace("/", "\,"))
#print(slashpath, "jfdhogiehaofihpg")
    sarrpath = [ ]

    print("Please enter the Path of the dictionary you want to evaluate")
    urpath = input()
    if os.path.lexists(urpath) == True:
        sarrpath.append(urpath)
        #print(sarrpath)
        #print("+++++")
        n=sarrpath[0].replace("\,\," , "/,/,")
        #print("n=", n)
        for i in range(len(subpaths)):
            if subpaths[i] in n:
                print(subpaths[i])
