...
data_folderMxSp = Path("GROBID_Evaluation/")
...
def generate_figure_per_feature (arrDictModel, kk):
    ...
    arrPaths= [filePathMxSp]
    # print(filePathDLF)
    for pp in arrPaths:
        # labels = {"lemma":"-1", "etym":"-1", "sense":"-1", "inflected":"-1", "note":"-1", "re":"-1", "usg":"-1", "xr":"-1","dictScrap":"-1"}
        if arrDictModel == "date":
            modelLabels = ["day", "month", "year"]
            labels = {"day":"-1", "month":"-1", "year":"-1"}
    ...
    ax.legend((rects1[0], rects1[0]), ('MxSp ('+mx_ma+')', 'MxSp ('+mx_ma+')'), loc='center left', bbox_to_anchor=(0.94, 0.4), prop={'size': 18}, title="Macro-average", title_fontsize="19")
    ...
    fig.savefig(f'figures/Histo_{arrDictModel+kk}.png', dpi=100)
    plt.close(fig)
